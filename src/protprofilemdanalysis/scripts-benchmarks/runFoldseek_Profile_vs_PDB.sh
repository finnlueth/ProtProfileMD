#!/bin/bash
"""
bash ./src/protprofilemdanalysis/scripts-benchmarks/runFoldseek_Profile_vs_PDB.sh \
...
--pdb-dir ./tmp/data/benchmarks/scop40pdb \
--profiles ./tmp/runs/ProtProfileMD_20251113_072928_large_batch_size_again/benchmarks/scope_40_benchmark/scope40_profiles.tsv \
--data-script-dir ./src/protprofilemdanalysis/scripts-data \
--out-dir ./tmp/testing/profile_vs_pdb
"""
...
usage() {
    cat << EOF
Usage: $0 -d SCOP_DIR -a AA_FASTA -t PDB_DIR -p PROFILE -b RUN_NAME
...
REQUIRED OPTIONS:
    ...
    -i, --pdb-dir DIR           Path to PDB directory
...
DB_STRUCTURE=${TEMP_DIR}/alignResults/db_structure
...
echo "Generating FoldSeek Structure DB"
foldseek createdb ${SEQUENCE_PDB_DIR} ${DB_STRUCTURE}/${DB_NAME_FOLDSEEK}

echo "Generating FoldSeek Sequence DB"
...
${FOLDSEEK} search ${DB_PROFILE}/${DB_NAME_FOLDSEEK}_profile ${DB_STRUCTURE}/${DB_NAME_FOLDSEEK} ${ALIGN_DIR_ALN} ${ALIGN_DIR_TMP} -s 9.5 --max-seqs 2000 -e 10 --sort-by-structure-bits 0

echo "Converting FoldSeek alignments"
${FOLDSEEK} convertalis ${DB_PROFILE}/${DB_NAME_FOLDSEEK}_profile ${DB_STRUCTURE}/${DB_NAME_FOLDSEEK} ${ALIGN_DIR_ALN} ${ALIGN_DIR_RAW}/foldseekaln
